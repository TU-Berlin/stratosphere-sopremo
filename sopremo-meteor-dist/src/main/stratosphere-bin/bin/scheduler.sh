...
log=$STRATOSPHERE_LOG_DIR/sopremo-$STRATOSPHERE_IDENT_STRING-scheduler.log
pid=$STRATOSPHERE_PID_DIR/sopremo-$STRATOSPHERE_IDENT_STRING-scheduler.pid
        ...
        (start)
                ...
                if [ -f $pid ]; then
                        if kill -0 `cat $pid` > /dev/null 2>&1; then
                                echo Scheduler running as process `cat $pid`.  Stop it first.
                                exit 1
                        fi
                fi
